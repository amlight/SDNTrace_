...
# From s1 to s7 - ok
ovs-ofctl add-flow s1 "in_port=1 actions=output:2" $PROTO
ovs-ofctl add-flow s2 "in_port=1 actions=output:3" $PROTO
ovs-ofctl add-flow s4 "in_port=4 actions=output:3" $PROTO
ovs-ofctl add-flow s6 "in_port=1 actions=output:2" $PROTO
ovs-ofctl add-flow s7 "in_port=1 actions=output:2" $PROTO
ovs-ofctl add-flow s8 "in_port=2 actions=output:1" $PROTO

# From s7 to s6 - ok
#ovs-ofctl add-flow s7 "in_port=1 actions=output:2" $PROTO

# From s7 to s5 - loop
#ovs-ofctl add-flow s6 "in_port=3 actions=output:2" $PROTO
#ovs-ofctl add-flow s5 "in_port=3 actions=output:3" $PROTO