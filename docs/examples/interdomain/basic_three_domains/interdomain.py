...
def InterDomain():
    ...
    # AmLight
    s16 = net.addSwitch('s16', listenPort=6616, mac='06:00:00:00:00:16')
    s17 = net.addSwitch('s17', listenPort=6617, mac='06:00:00:00:00:17')
    s18 = net.addSwitch('s18', listenPort=6618, mac='06:00:00:00:00:18')

    h6 = net.addHost('h6', mac='dd:00:00:00:00:16')
    net.addLink(s16, s17, port1=1, port2=1)
    net.addLink(s16, s18, port1=2, port2=3)
    net.addLink(s17, s18, port1=2, port2=2)
    net.addLink(h6, s18, port1=1, port2=1)

    amlightctl = net.addController('amlightcontroller', controller=RemoteController,
                                   ip='190.103.187.55', port=9903)

    # Inter-domain
    net.addLink(s4, s6, port1=3, port2=1)
    net.addLink(s8, s17, port1=3, port2=3)
    ...
    s7.start([claractl])
    s8.start([claractl])
    s16.start([amlightctl])
    s17.start([amlightctl])
    s18.start([amlightctl])
